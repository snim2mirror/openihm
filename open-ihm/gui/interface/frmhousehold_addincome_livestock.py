...
class FrmHouseholdLivestockIncome(QDialog, Ui_AddHouseholdIncomeLivestock):	
    ...
    def displayIncomeDetails(self):
		...
		query = '''SELECT * FROM livestockincome WHERE hhid=%s AND pid=%s AND id=%s ''' % ( self.hhid, self.pid, self.incomeid )
    ...
    def saveIncome(self):
		...
		# create UPDATE query
		if (self.incomeid == 0):
			query = '''INSERT INTO livestockincome (hhid, incomesource, unitofmeasure, unitsconsumed, unitssold, unitprice, pid )
